...
!! empty then output by default goes to `stdout`. Otherwise reports
!! go to `stdout` only if it has been explicitly added to `log_units`.
...
!!
!! The variable `global_logger` of type `logger_t` can be used
...
!! indicates that at least one of the writes to `log_units` failed.
    ...
    type :: logger_t
        ...
        !! Public derived type ([Specification](../page/specs/stdlib_logger.html#the-derived-type-logger_t))
        private
    ...
    contains
        ...
        procedure, public :: add_log_file
        procedure, public :: add_log_unit
        procedure, public :: configuration
        procedure, public :: configure
        procedure, public :: log_debug
        procedure, public :: log_error
        procedure, public :: log_information
        procedure, public :: log_io_error
        procedure, public :: log_message
        procedure, public :: log_text_error
        procedure, public :: log_units_assigned
        procedure, public :: log_warning
        procedure, public :: remove_log_unit
        ...
        if (len_trim(aaction) == 4) then
            ...
            if (aaction == 'read') then
                if (present(stat)) then
                    ...
                    call error_stop('In '//module_name//' % '// &
...
999     if (present(stat)) then
            ...
            call self%log_io_error('Unable to open '//trim(filename), &
                                   ...
                                   iostat=iostat, &
                                   iomsg=iomsg)
            call error_stop(module_name//' % '//procedure_name// &
                            ': Unable to open file.')
        end if
...
! Check that unit is not stdin
            if (unit == stdin) then
                if (present(stat)) then
                    stat = read_only_error
                    return
                else
                    call error_stop('unit in '//module_name//' % '// &
                                    procedure_name//' must not be stdin.')
                end if
            end if
...
!! Finalizes the `logger_t` entity `self` by flushing the units
        ...
        do unit = 1, self%units
            ...
            if (iostat /= 0) then
                write (stderr, '(a, i0)') 'In the logger_t '// &
                    'finalizer an error occurred in flushing unit = ', &
                    self%log_units(unit)
                write (stderr, '(a, i0)') 'With iostat = ', iostat
                write (stderr, '(a)') 'With iomsg = '//trim(message)
            end if
        end do
...
!! `iomsg` by writing a description of the failure to `stdout` and
!! stopping.
        integer, intent(in) :: unit
        ...
        write (stdout, '(a)') 'write failure in '//module_name// &
            ' % '//trim(procedure_name)//'.'
        if (unit == -999) then
            write (stdout, '(a, i0)') 'unit = internal file'
        else
            write (stdout, '(a, i0)') 'unit = ', unit
            inquire (unit, named=named)

            if (named) then
                inquire (unit, name=name)
                write (stdout, '(a, a)') 'name = ', trim(name)
            else
                write (stdout, '(a)') 'unit is unnamed'
            end if
            inquire (unit, action=action)
            write (stdout, '(a, a)') 'action = ', trim(action)
        end if

        write (stdout, '(a, i0)') 'iostat = ', iostat
        write (stdout, '(a, a )') 'iomsg = ', trim(iomsg)
        call error_stop(module_name//' % '//trim(procedure_name)//'.')
...
999     if (present(stat)) then
            ...
            call error_stop('close_unit failed in '//module_name//' % '// &
                            procedure_name//'.')
        end if

    end subroutine remove_log_unit

end module seakeeping_logger
